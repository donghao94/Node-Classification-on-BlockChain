...
class AttnModel(torch.nn.Module):
    ...
    """
    def __init__(self, feat_dim, edge_dim, time_dim, 
                 attn_mode='prod', n_head=2, drop_out=0.1):           ############## n_head = 2
        """
        args:
    ...
    def forward(self, src, src_t, seq, seq_t, seq_e, mask):
        ...
        src_ext = torch.unsqueeze(src, dim=1)  # src [B, 1, D] 在dim=1 增加一个维度
        src_e_ph = torch.zeros_like(src_ext)  # torch.zeros_like():生成和括号内变量维度维度一致的全是零的内容--> zeros[B,1,D]
        ...
        output = output.squeeze()  # 去掉维度是1的那个纬度
        attn = attn.squeeze()
